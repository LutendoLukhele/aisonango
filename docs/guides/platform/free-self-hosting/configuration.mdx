...
## Internal-first setup (cache + sync + proxy)

For internal tools, a practical starting point is to keep Connect UI enabled while your own internal UI is still being built:
...
Then expose only the APIs you need in your internal backend:
- `/proxy/*` for authenticated API proxy calls
- `/sync/*` for triggering/pausing/status of syncs
- `/records` for reading cached sync data

You can later disable Connect UI once your custom UI fully replaces it:

```sh
FLAG_SERVE_CONNECT_UI=false
```

## Runtime timeout and backpressure

If your function runtime is exceeding your desired limits (for example 50 minutes), set an explicit runner timeout and cap concurrency:

```sh
RUNNER_EXECUTION_TIMEOUT_SECS=3000
SYNC_ENVIRONMENT_MAX_CONCURRENCY=50
ACTION_ENVIRONMENT_MAX_CONCURRENCY=50
WEBHOOK_ENVIRONMENT_MAX_CONCURRENCY=25
ON_EVENT_ENVIRONMENT_MAX_CONCURRENCY=25
```

`RUNNER_EXECUTION_TIMEOUT_SECS` is applied to self-hosted non-Lambda runners.  
When the limit is reached, the runner sends an abort signal to the execution and the task is reported as a failed execution.

## Can it run on GitHub?

You can use GitHub for source control, CI/CD, and image publishing, but GitHub is not a runtime host for Nango services.  
Run your self-hosted instance on your own infrastructure (VM, Kubernetes, or managed compute platform).
