...
If your function runtime is exceeding your testing limits (for example 50 minutes), set an explicit runner timeout and cap concurrency:
...
`RUNNER_EXECUTION_TIMEOUT_SECS` is applied to self-hosted non-Lambda runners and helps prevent long-running executions from growing unbounded.
